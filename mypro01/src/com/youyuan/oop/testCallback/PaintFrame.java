...
public class PaintFrame {
	public static void drawFrame(MyFrame f) {//继承方式，普通类和抽象类
//	public static void drawFrame(IMyFrame f) {//接口方式实现
		System.out.println("第一步");
		System.out.println("第二步");
		System.out.println("第三步");
		//调用方法
		f.paint();//调用类(MyFrame)型对象f的方法
		System.out.println("第五步");
		System.out.println("第六步");
		
	}
	public static void main(String[] args) {
		drawFrame(new MyFrame());//普通类调用
		System.out.println("**************");
		drawFrame(new GameFrame01());	//实现重写抽象类方法的调用
	}
...
class GameFrame01 extends MyFrame{
	public void paint() {
		System.out.println("通过抽象类继承的方式，实现对父类方法的重写");
